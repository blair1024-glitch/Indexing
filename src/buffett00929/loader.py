...
class DataLoader:
    ...
    def load_company(
        ...
        # --- 2. 產業別與市場別 -------------------------------------------------
        # 要在 _overlay_official **之前**：市場別決定去問證交所還是櫃買中心，
        # 選錯就整批對不到。掃描的第二階段與個股查詢都走
        # build_lookup_constituent，那條路徑沒有市場別；而市場別原本只在
        # classify() 裡決定，classify 又綁 ConstituentSet，只有每日 00929
        # 流程會呼叫——所以那兩條路徑一律被當成上市。
        #
        # 實測（2026-08-27 全市場掃描）：達爾膚 6523 是**上櫃**，卻被拿去問
        # TWSE，報表留下「TWSE: 找不到欄位 PEratio／殖利率／收盤價」，
        # 本益比、股價淨值比、殖利率整排變成資料不足，市場別還印成「上市」。
        # 產業別同理：原本只有第一階段補，所以品質前 50 名——最重要的那些——
        # 反而全部顯示「未分類」。
        self._apply_directory(company)

        # --- 3. FinMind 補 MOPS 沒有的欄位 ------------------------------------
