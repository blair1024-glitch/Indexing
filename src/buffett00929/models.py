...
STAGE_ONE_GAP = "此為全市場掃描第一階段"
"""第一階段（只用彙總報表）留下的缺料註記開頭。

由 ``loader`` 寫入、``report`` 讀取，用來區分兩件會長得一模一樣、
意義卻相反的事：**沒有去算**（第一階段不補估值）與**算不出來**
（補了資料但方法不足或彼此分歧）。報表把兩者混為一談，
讀者就會以為這家公司估不出價，而不是我們還沒估。
"""
